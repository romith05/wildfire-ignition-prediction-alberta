"""Validate Model B 1 km feature config before geospatial extraction.

This utility checks that a local feature config is compatible with the trained
Model B normalization statistics. It is meant to catch costly mistakes before
creating Alberta-wide inference patches.
...
- raster paths exist
- raster band numbers are valid
- constant weather features have numeric values
...
def validate_against_channel_stats(config_keys: list[str], channel_stats: dict[str, Any]) -> None:
    """Compare config feature keys against training channel statistics."""
    ...
    if config_keys != stats_keys:
        ...
        message = [
            ...
            "Fix configs/model_b_1km_features.json so keys exactly match training feature_keys.",
        ]
        raise ValueError("\n".join(message))


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Validate Model B 1 km feature config.")
    parser.add_argument("--feature-config", required=True, help="Local model_b_1km_features.json path.")
    parser.add_argument("--channel-stats", required=True, help="1 km channel_stats.json used by Model B.")
...
def main() -> None:
    ...
    print("Model B feature config validation passed")
